...
function MyVehicles() {
  ...
  useEffect(()=>{ load(); },[]);
  async function create(e){ e.preventDefault(); await fetch('/api/client/vehicles',{ method:'POST', headers: headers(), body: JSON.stringify(form)}); setForm({ make:'', model:''}); load(); }
  return (
    <div>
      <h3>My Vehicles</h3>
...
function Feedback() {
  ...
  async function submit(e){
    e.preventDefault(); setMsg('');
    try {
      const res = await fetch('/api/client/feedback', { method:'POST', headers: headers(), body: JSON.stringify(form)});
      const data = await res.json().catch(()=>({}));
